...
设备指纹（Device Fingerprint）是一种用于识别和追踪设备的技术，它通过收集设备唯一标识（如设备id、MAC地址、IMEI等）和行为特征（如浏览器版本、操作系统版本、屏幕分辨率等）来生成一个独特的设备标识符。
...
我们可以把这类设备指纹称为bncode（Browser And Network Code）。他结合了设备的浏览器属性和网络属性，生成一个简短的字符串，bncode数据通常包含以下信息：

- 浏览器信息：浏览器类型、版本、UA等
- 网络信息：IP地址、子网掩码、DNS服务器等
- 设备信息：设备类型、操作系统版本、屏幕分辨率等
